...
public final class StringHelper {
     ...
     * @param string
     *            UTF-8 string
     * @return byte array created from given string, or empty array if null or
     *         empty string was provided
     */
    public static byte[] getUtf8ByteArray(String string) {
	return hasContent(string) ? string.getBytes(Charset
		.forName(UTF8_CHARSET_NAME)) : new byte[0];
    }
     ...
     * @return UTF-8 string created from given array, or NULL if NULL array was
     *         provided
     */
    public static String toUtf8String(byte[] array) {
	return array != null ? new String(array,
		Charset.forName(UTF8_CHARSET_NAME)) : null;
    }
	...
	try {
	    ...
	    throw new StringHelperException(exc,
		    ErrorMessages.STRING_HELPER_CANNOT_ENCODE, url,
     ...
     * @param stringOne first string
     * @param stringTwo second string
     * @param ignoreCase true, if case-sensitivity should be ignored, false otherwise
     * @return true, if strings are equal, false otherwise
     */
    private static boolean equals(String stringOne, String stringTwo,
	    boolean ignoreCase) {
