...
/**
 * This stub is a fake repository. It memorizes all that it sees, records
 * nothing on disk, and reply to data requests according to what it has in
 * memory. Remark: a request for objects of a class is considered stricly,
 * objects of the subclasses are not returned. It can be considered as a bug,
 * indeed, but this is only a fake repository for now and no need for better has
 * been recorded up to now. <br>
 * Note for understanding the code: the internal stocking of data is
 * hierarchical, using quite arbitrary ordering to optimize the requests. Thus
 * the getData() method has been split in one per hierarchy layer.
 * <p>
 * This module is coded by extending ModuleImpl, which is a generic
 * multi-purpose Module shell.
 * 
 * @author F270116
 * 
 */
public class RepositoryStubService extends ModuleImpl implements
	...
	/**
	 * Sort by date first, then by source, then by ID. Null are allowed, but
	 * there should be no null !
	 * 
	 * @author F270116
	 * 
	 */
	protected class AbstractDataFusionTypeSortByDate implements
			Comparator<AbstractDataFusionType> {

		@Override
		public int compare(AbstractDataFusionType left,
				AbstractDataFusionType right) {
	...
	/**
	 * registers the module as Repository, obviously, but also as DataConsumer
	 * since it needs to listen to every data to record them.
	 */
	public void registerService() {
		_client.registerModule(this);
		_client.registerModuleAsRepository(this);
		_client.registerModuleAsDataConsumer(this, null);
	}
	...
	private List<AbstractDataFusionType> getData(
			TreeSet<AbstractDataFusionType> dataSet, DataFilter filter) {
		List<AbstractDataFusionType> returnedObjects = new ArrayList<AbstractDataFusionType>();
		// following trick is dirty. Pure souls, please do turn your eyes elsewhere !
		// ;)
		Threat startDummy = new Threat();
		startDummy.setTimestamp(filter.getStartDate() == null ? 0l : filter
				.getStartDate());
		Threat endDummy = new Threat();
		endDummy.setTimestamp(filter.getEndDate() == null ? Long.MAX_VALUE
				: filter.getEndDate());
	...
	private List<AbstractDataFusionType> getData(
			HashMap<String, TreeSet<AbstractDataFusionType>> classMap,
			DataFilter filter) {
		List<AbstractDataFusionType> returnedObjects = new ArrayList<AbstractDataFusionType>();
		if (filter.getDataFusionType() == null) {
			...
			TreeSet<AbstractDataFusionType> dataSet = classMap.get(filter
					.getDataFusionType());
			if (dataSet != null) {
				returnedObjects.addAll(getData(dataSet, filter));
			}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.safran.arena.impl.ModuleImpl#getData(java.lang.Class,
	 * java.lang.String, java.util.List)
	 */
	@Override
	public List<AbstractDataFusionType> getData(Class<?> type,
			String dataSourceID, List<DataFilter> filters) {

		// iterate on filters
		// for each filter, apply datasourcefilter and call specialised getData
		// on resulting maps
		List<AbstractDataFusionType> returnedObjects = super.getData(type,
				dataSourceID, filters);
		synchronized (_DB) {
			for (DataFilter filter : filters) {
				if (filter.getProducerId() == null) {
					for (HashMap<String, TreeSet<AbstractDataFusionType>> classMap : _DB
							.values()) {
						returnedObjects.addAll(getData(classMap, filter));
					}
				} else {
					HashMap<String, TreeSet<AbstractDataFusionType>> classMap = _DB
							.get(filter.getProducerId());
					if (classMap != null) {
						returnedObjects.addAll(getData(classMap, filter));
					}
				}
	 ...
	 */
	@Override
	public void onDataAvailable(Class dataType, String dataSourceId,
		...
		synchronized (_DB) {
			...
			TreeSet<AbstractDataFusionType> dataSet = classMap.get(dataType
					.getSimpleName());
			if (dataSet == null) {
				dataSet = new TreeSet<AbstractDataFusionType>(
						_objectSorterByDate);
	 ...
	 */
	@Override
	public void onDataChanged(Class dataType, String dataSourceId,
			AbstractDataFusionType data) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.safran.arena.impl.ModuleImpl#onDataDeleted(java.lang.Class,
	 * java.lang.String, eu.arena_fp7._1.AbstractDataFusionType)
	 */
	@Override
	public void onDataDeleted(Class dataType, String dataSourceId,
			AbstractDataFusionType data) {
	...
	public static void main(String[] args) {
		RepositoryStubService stub = new RepositoryStubService(
				"Repository Stub " + /*Math.random()*/ "RP");
		stub.init();
