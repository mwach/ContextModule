 ...
 */
/**
 * Accepts position updates from connected GPS device Example input data:
 * {"Latitude"
 * :53.11581257265061,"Longitude":17.9995668400079,"Altitude":108.0999755859375
 * ,"Bearing"
...
public class GpsListener implements HttpRequestHandler, LocationPublisher {
    ...
    static final byte[] GET_REQUEST_ANY = StringHelper
	    .getUtf8ByteArray("Only POST requests are supported");
    static final byte[] POST_RESPONSE_OK = StringHelper.getUtf8ByteArray("OK");
    static final byte[] POST_RESPONSE_FAIL = StringHelper
	    .getUtf8ByteArray("EMPTY");
	...
	try {
	    newLocation = IOHelper.readStreamData(request.getInputStream(),
		    Constants.ENCODING);
	    LogHelper.debug(GpsListener.class, "doPost", "Received data: '%s'",
		    newLocation);

	    // write confirmation to the output
	    response.getOutputStream().write(
		    StringHelper.hasContent(newLocation) ? POST_RESPONSE_OK
			    : POST_RESPONSE_FAIL);

	    notifyListener(newLocation);

	} catch (IOHelperException e) {
	    LogHelper.warning(GpsListener.class, "doPost",
		    "Could not process the request: %s",
	...
	LogHelper.debug(GpsListener.class, "doGet",
		"GET request detected from address: '%s'", req.getRemoteAddr());
