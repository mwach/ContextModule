...
/**
 * Class mocking GPS behavior Produces GPS coordinates compatible with the
...
public class GpsListenerMock implements LocationPublisher {
	...
	try {
		...
		throw new GeoportalException(
			ErrorMessages.GEOPORTAL_NULL_START_LOCATION);
	    }
	    if (getStartLocation() == null) {
		throw new GeoportalException(
			ErrorMessages.GEOPORTAL_NULL_START_LOCATION);
	    ...
	    LogHelper.exception(GpsListenerMock.class, "init",
		    "Could not initialize component", exc);
	...
	this.deltaLocation = new PlatformLocation(startLocation.getId(),
		longitude, latitude, altitude, 0, 0, 0, 0);
    }
	...
	double altitude = getStartLocation().getAltitude() + currentStep
		* deltaLocation.getAltitude();
	double longitude = getStartLocation().getLongitude() + currentStep
		* deltaLocation.getLongitude();
	double latitude = getStartLocation().getLatitude() + currentStep
		* deltaLocation.getLatitude();
	...
	PlatformLocation returnLocation = new PlatformLocation(
		getStartLocation().getId(), longitude, latitude, altitude,
