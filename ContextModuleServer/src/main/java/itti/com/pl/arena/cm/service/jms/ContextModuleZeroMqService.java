...
public class ContextModuleZeroMqService extends ModuleImpl implements ContextModule, Service {
	...
	try {
	    ...
	    LogHelper.error(ContextModuleZeroMqService.class, "init", "Validation failed: %s", exc.getLocalizedMessage());
	    throw new BeanInitializationException(exc.getLocalizedMessage());
	} catch (NetworkHelperException exc) {
	    LogHelper.exception(ContextModuleZeroMqService.class, "init", "Could not set clientIpAddress", exc);
	    throw new BeanInitializationException(exc.getLocalizedMessage());
	} catch (RuntimeException exc) {
	    LogHelper.exception(ContextModuleZeroMqService.class, "init", "Could not initialize ContextModule", exc);
	    throw new BeanInitializationException(exc.getLocalizedMessage());
	}
    }
	...
	try {
	    ...
	    LogHelper.exception(ContextModuleZeroMqService.class, "shutdown", "Could not shutdown the ContextModule", exc);
	}
    }
	...
	try {
		...
		if (StringHelper.equalsIgnoreCase(ContextModuleRequests.getPlatform.name(), data.getHref())
		    ...
		    LogHelper.info(ContextModuleZeroMqService.class, "onDataChanged", "Invalid method requested: '%s'",
			    data.getHref());
		}
	...
	List<GeoObject> geographicalInformation = getContextManager().getGISData(location.getX(), location.getY());
	...
	List<GeoObject> geoportalInformation = getContextManager().getGeoportalData(location.getX(), location.getY());
